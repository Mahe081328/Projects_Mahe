...
state = 'tamilnadu'
def scrap_data(date):
  url = f"https://vegetablemarketprice.com/api/dataapi/market/{state}/daywisedata?date={date}"
  header = {
      "accept": "*/*",
  ...
  js_data = json.loads(status.text)
...
all_data = [{"State": "Tamil Nadu"}]
for date in dates:
    daily_data = scrap_data(date)
    all_data.extend(daily_data)

df = pd.DataFrame(all_data)
df.to_csv("Vegetable_Details_TN.csv", index = False)

print("Data Scrapped and save to 'Vegetable_Details_TN' ")
